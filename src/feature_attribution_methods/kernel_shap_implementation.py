...
def to_inputs(tensor, input_docs):
    ...
    for i in range(len(encoded)):
        ...
        for j in range(len(encoded[i]) - 2):
            ...
            if padded.data[index] != 0:
                temp.append(tokenizer.decode(padded.data[index]).replace(' ', ''))
            index += 1
        result.append(temp)
    return result
...
def shap(shap_tensor, shap_queries, shap_docs, shap_labels, target, n_samples):
    """
    The Kernel SHAP is defined here (from the library Captum).

    Args:
        shap_tensor: The docs from the data in tensor form.
        shap_queries: The queries from the data.
        shap_docs: The docs from the data.
        shap_labels: The label from the data.
    ...
    """
    kernel_shap = KernelShap(wrapper)
    attributions = kernel_shap.attribute(shap_tensor,
                                         target=target,
                                         additional_forward_args=(shap_queries, shap_docs, shap_labels),
                                         n_samples=n_samples,
                                         show_progress=True)
    return attributions
...
def marked_sentence(c_tokens, fa, name):
    ...
    with open('/dir/to/your/colorized_kernel_shap_' + name + '.html', 'w') as f:
...
if __name__ == '__main__':
    ...
    with open("/dir/to/your/kernel_shap_" + str(num) + ".json", "w") as f:
        ...
        attribution = shap(tensor_docs, queries, docs, labels, int(numpy.argmax(classification)), iterations)
