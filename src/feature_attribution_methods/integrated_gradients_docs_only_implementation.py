...
def wrapper(wrapper_tensor_docs, wrapper_queries=None):
    ...
    """
    wrapper_tensor_queries = to_tensor(wrapper_queries)
    tensor = torch.cat((torch.tensor([tokenizer.cls_token_id]), wrapper_tensor_queries,
                        torch.tensor([tokenizer.sep_token_id]), wrapper_tensor_docs.data[0],
                        torch.tensor([tokenizer.sep_token_id])))
    output = model.cls_module.forward(tensor.to(torch.int64).unsqueeze(0))  # The output
    return output.get('cls_pred')


def ig(ig_tensor, ig_queries, target, n_samples):
    """
    The Layer Integrated Gradients is defined here (from the library Captum).

    Args:
        ig_tensor: The docs from the data in tensor form.
        ig_queries: The queries from the data.
        ig_docs: The docs from the data.
        ig_labels: The label from the data.
    ...
    """
    int_gr = LayerIntegratedGradients(wrapper, model.cls_module.bare_bert.embeddings)
    attributions = int_gr.attribute(ig_tensor.unsqueeze(0),
                                    baselines=torch.zeros(len(ig_tensor), device=device).unsqueeze(0),
                                    target=target,
                                    additional_forward_args=ig_queries,
                                    n_steps=n_samples)
    return attributions
...
def marked_sentence(c_tokens, fa, name):
    ...
    with open('/dir/to/your/colorized_integrated_gradients_' + name + '.html', 'w') as f:
...
if __name__ == '__main__':
    ...
    with open("/dir/to/your/integrated_gradients_" + str(num) + ".json", "w") as f:
